...
// externally-supplied token symbols so a crafted symbol can't break rendering (→ Telegram 400).
...
export function formatWatchlist(entries: WatchlistEntry[], treasurySizeBnb: number): string {
  ...
  entries.forEach((e, i) => {
    lines.push(`${i + 1}. ${GATE_ICON[e.gate]} *${safeSymbol(e.candidate.tokenSymbol)}* — grade ${e.grade} · momentum ${e.candidate.momentumScore}`);
  });
...
export function formatWatchlistEntry(entry: WatchlistEntry, explanation: string): string {
  const lines = [
    `${GATE_ICON[entry.gate]} *${safeSymbol(entry.candidate.tokenSymbol)}* — grade ${entry.grade} (${entry.gate})`,
    explanation,
    "",
